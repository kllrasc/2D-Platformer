...
func _physics_process(_delta):
	...
	if $AnimatedSprite2D.animation == "Damage": $AnimatedSprite2D.offset.x = 7*direction
...
func damage():
	if SM.state_name != "PROPERTY_HINT_NODE_PATH_TO_EDITED_NODE":
		SM.set_state("Death")
...
func _on_AnimatedSprite_animation_finished():
	if SM.state_name == "Damage":
		SM.set_state("Idle")
	if SM.state_name == "Death":
		queue_free()


func _on_Above_and_Below_body_entered(body):
	if body.name == "Player" and SM.state_name != "Death":
		body.die()
